...
void CharacterDatabaseConnection::DoPrepareStatements()
{
    ...
    */
    ...
    PrepareStatement(CHAR_SEL_SUM_CHARS, "SELECT COUNT(guid) FROM characters WHERE account = ? AND deleteDate IS NULL", CONNECTION_BOTH); //sun: exclude pre deleted characters
    PrepareStatement(CHAR_SEL_CHAR_CREATE_INFO, "SELECT level, race, class FROM characters WHERE account = ? AND deleteDate IS NULL LIMIT 0, ?", CONNECTION_ASYNC);
    /*
    ...
    */
    ...
    PrepareStatement(CHAR_SEL_ENUM_DECLINED_NAME, "SELECT c.guid, c.name, c.race, c.class, c.gender, c.playerBytes, c.playerBytes2, c.level, c.zone, c.map, "
                     ...
                     "cd.genitive FROM characters AS c LEFT JOIN character_pet AS cp ON c.guid = cp.owner AND cp.slot = ? "
    ...
    */
    ...
    PrepareStatement(CHAR_SEL_CHARACTER_QUESTSTATUS, "SELECT quest,status,explored,timer,mobcount1,mobcount2,mobcount3,mobcount4,itemcount1,itemcount2,itemcount3,itemcount4,playercount FROM character_queststatus WHERE guid = ?", CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_SEL_CHARACTER_SOCIALLIST, "SELECT friend,flags,note FROM character_social WHERE guid = ? LIMIT 255", CONNECTION_ASYNC);
    ...
    */
    PrepareStatement(CHAR_INS_MAIL, "INSERT INTO mail(id, messageType, stationery, mailTemplateId, sender, receiver, subject, itemTextId, has_items, expire_time, deliver_time, money, cod, checked) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", CONNECTION_ASYNC);
    //diff with TC here, delete from item_text too if any
    PrepareStatement(CHAR_DEL_MAIL_BY_ID, "DELETE m.*, it.*FROM mail m LEFT JOIN item_text it ON  m.itemTextId = it.id WHERE m.id = ?", CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_DEL_EMPTY_EXPIRED_MAIL, "DELETE FROM mail WHERE expire_time < ? AND has_items = 0 AND itemTextId = 0", CONNECTION_ASYNC);
    ...
    */       
    //Update CHAR_SEL_ITEM_INSTANCE_FIELDS_COUNT if you change itemCommonPart
    //                            0                                              4                                                                               9      10                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          43             44              45          46
    std::string itemCommonPart = "creatorGuid, giftCreatorGuid, count, duration, spell1_charges, spell2_charges, spell3_charges, spell4_charges, spell5_charges, flags, enchant1_id, enchant1_duration, enchant1_charges, enchant2_id, enchant2_duration, enchant2_charges, enchant3_id, enchant3_duration, enchant3_charges, enchant4_id, enchant4_duration, enchant4_charges, enchant5_id, enchant5_duration, enchant5_charges, enchant6_id, enchant6_duration, enchant6_charges, enchant7_id, enchant7_duration, enchant7_charges, enchant8_id, enchant8_duration, enchant8_charges, enchant9_id, enchant9_duration, enchant9_charges, enchant10_id, enchant10_duration, enchant10_charges, enchant11_id, enchant11_duration, enchant11_charges, property_seed, random_prop_id, durability, itemTextId";
    //                                                                             47   48
    PrepareStatement(CHAR_SEL_CHARACTER_INVENTORY, "SELECT " + itemCommonPart + ", bag, slot, "
    ...
    //                                                                  47         48         49                                
    PrepareStatement(CHAR_SEL_MAILITEMS, "SELECT "+ itemCommonPart + ", item_guid, itemEntry, owner_guid FROM mail_items mi JOIN item_instance ii ON mi.item_guid = ii.guid WHERE mail_id = ?", CONNECTION_SYNCH);
    PrepareStatement(CHAR_SEL_ITEM_INSTANCE, "SELECT " + itemCommonPart + " FROM item_instance WHERE guid = ?", CONNECTION_SYNCH);
    PrepareStatement(CHAR_SEL_AUCTION_ITEMS, "SELECT " + itemCommonPart + ", itemguid, itemEntry FROM auctionhouse ah JOIN item_instance ii ON ah.itemguid = ii.guid", CONNECTION_SYNCH);
    PrepareStatement(CHAR_SEL_GUILDBANK_ITEMS, "SELECT " + itemCommonPart + ", TabId, SlotId, item_guid, itemEntry FROM guild_bank_item gbi INNER JOIN item_instance ii ON gbi.item_guid = ii.guid where guildid = ?", CONNECTION_ASYNC);
    //next two (CHAR_REP_ITEM_INSTANCE and CHAR_UPD_ITEM_INSTANCE) must use exactly the same fields
    PrepareStatement(CHAR_REP_ITEM_INSTANCE, "REPLACE INTO item_instance (guid, owner_guid, itemEntry, container_guid, creatorGuid, giftCreatorGuid, count, duration, spell1_charges, spell2_charges, spell3_charges, spell4_charges, spell5_charges, flags, enchant1_id, enchant1_duration, enchant1_charges, enchant2_id, enchant2_duration, enchant2_charges, enchant3_id, enchant3_duration, enchant3_charges, enchant4_id, enchant4_duration, enchant4_charges, enchant5_id, enchant5_duration, enchant5_charges, enchant6_id, enchant6_duration, enchant6_charges, enchant7_id, enchant7_duration, enchant7_charges, enchant8_id, enchant8_duration, enchant8_charges, enchant9_id, enchant9_duration, enchant9_charges, enchant10_id, enchant10_duration, enchant10_charges, enchant11_id, enchant11_duration, enchant11_charges, property_seed, random_prop_id, durability, itemTextId) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)", CONNECTION_ASYNC);
    PrepareStatement(CHAR_UPD_ITEM_INSTANCE, "UPDATE item_instance SET guid = ? , owner_guid = ? , itemEntry = ? , container_guid = ? , creatorGuid = ? , giftCreatorGuid = ? , count = ? , duration = ? , spell1_charges = ? , spell2_charges = ? , spell3_charges = ? , spell4_charges = ? , spell5_charges = ? , flags = ? , enchant1_id = ? , enchant1_duration = ? , enchant1_charges = ? , enchant2_id = ? , enchant2_duration = ? , enchant2_charges = ? , enchant3_id = ? , enchant3_duration = ? , enchant3_charges = ? , enchant4_id = ? , enchant4_duration = ? , enchant4_charges = ? , enchant5_id = ? , enchant5_duration = ? , enchant5_charges = ? , enchant6_id = ? , enchant6_duration = ? , enchant6_charges = ? , enchant7_id = ? , enchant7_duration = ? , enchant7_charges = ? , enchant8_id = ? , enchant8_duration = ? , enchant8_charges = ? , enchant9_id = ? , enchant9_duration = ? , enchant9_charges = ? , enchant10_id = ? , enchant10_duration = ? , enchant10_charges = ? , enchant11_id = ? , enchant11_duration = ? , enchant11_charges = ? , property_seed = ? , random_prop_id = ? , durability = ?, itemTextId = ? WHERE guid = ?", CONNECTION_ASYNC);
    PrepareStatement(CHAR_UPD_ITEM_INSTANCE_ON_LOAD, "UPDATE item_instance SET guid = ? , owner_guid = ? , duration = ? , flags = ? , durability = ? WHERE guid = ?" , CONNECTION_ASYNC);
    ...
    */
    PrepareStatement(CHAR_SEL_CHARACTER_COUNT, "SELECT account, COUNT(guid) FROM characters WHERE account = ? AND deleteDate IS NULL GROUP BY account", CONNECTION_ASYNC);
    /*
 ...
 /*
    ...
    */
    ...
    PrepareStatement(CHAR_UPD_ARENA_TEAM_STATS, "UPDATE arena_team SET rating = ?, weekGames = ?, weekWins = ?, seasonGames = ?, seasonWins = ?, `rank` = ?, nonPlayedWeeks = ? WHERE arenaTeamId = ?", CONNECTION_ASYNC);
    ...
    */
    ...
    // GM Tickets
    PrepareStatement(CHAR_SEL_GM_TICKETS, "SELECT id, type, playerGuid, name, description, createTime, mapId, posX, posY, posZ, lastModifiedTime, closedBy, closedTime, assignedTo, comment, response, completed, escalated, viewed, needMoreHelp FROM gm_ticket", CONNECTION_SYNCH);
    PrepareStatement(CHAR_REP_GM_TICKET, "REPLACE INTO gm_ticket (id, type, playerGuid, name, description, createTime, mapId, posX, posY, posZ, lastModifiedTime, closedBy, closedTime, assignedTo, comment, response, completed, escalated, viewed, needMoreHelp, resolvedBy) VALUES (?,?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", CONNECTION_ASYNC);
    ...
    */
    ...
    PrepareStatement(CHAR_INS_CHARACTER, "INSERT INTO characters (guid, account, name, race, class, gender, level, xp, money, playerBytes, playerBytes2, playerFlags, "
        "map, instance_id, dungeon_difficulty, position_x, position_y, position_z, orientation, trans_x, trans_y, trans_z, trans_o, transguid, "
        "taximask, cinematic, "
        "totaltime, leveltime, rest_bonus, logout_time, is_logout_resting, resettalents_cost, resettalents_time, "
        "extra_flags, stable_slots, at_login, zone, "
        "death_expire_time, taxi_path, arenapoints, totalHonorPoints, todayHonorPoints, yesterdayHonorPoints, totalKills, "
        "todayKills, yesterdayKills, chosenTitle, watchedFaction, drunk, health, power1, power2, power3, "
        "power4, power5, latency, exploredZones, equipmentCache, ammoId, knownTitles, actionBars, xp_blocked) VALUES "
        "(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)", CONNECTION_ASYNC);
    PrepareStatement(CHAR_UPD_CHARACTER, "UPDATE characters SET name=?,race=?,class=?,gender=?,level=?,xp=?,money=?,playerBytes=?,playerBytes2=?,playerFlags=?,"
        ...
        "equipmentCache=?,ammoId=?,knownTitles=?,actionBars=?,xp_blocked=?,online=? WHERE guid=?", CONNECTION_ASYNC);
    ...
    */
    PrepareStatement(CHAR_UPD_DELETE_INFO, "UPDATE characters SET deleteInfos_Name = name, deleteInfos_Account = account, deleteDate = UNIX_TIMESTAMP(), name = '', at_login = at_login | '1' WHERE guid = ?", CONNECTION_ASYNC);
    /*
    ...
    */
    PrepareStatement(CHAR_INS_ADDON, "INSERT INTO addons (name, crc, gamebuild) VALUES (?, ?, ?)", CONNECTION_ASYNC);
    ...
    */
    ...
    PrepareStatement(CHAR_INS_INSTANCE_RESET, "INSERT INTO instance_reset (mapid, difficulty, resettime) VALUES(?, ?, ?)", CONNECTION_ASYNC);
    ...
    */
    ...
    PrepareStatement(CHAR_SEL_CHAR_PINFO, "SELECT totaltime, level, money, account FROM characters WHERE guid = ?", CONNECTION_SYNCH);
    /*
    ...
    */
    PrepareStatement(CHAR_SEL_CHAR_COD_ITEM_MAIL, "SELECT id, messageType, mailTemplateId, sender, subject, itemTextId, money, has_items FROM mail WHERE receiver = ? AND has_items <> 0 AND cod <> 0", CONNECTION_SYNCH);
    /*
    ...
    */
    PrepareStatement(CHAR_SEL_MAIL, "SELECT id, messageType, sender, receiver, subject, itemTextId, has_items, expire_time, deliver_time, money, cod, checked, stationery, mailTemplateId FROM mail WHERE receiver = ? ORDER BY id DESC", CONNECTION_SYNCH);
    /*
    ...
    */
    ...
    PrepareStatement(CHAR_DEL_EXPIRED_CHAR_INSTANCE_BY_MAP_DIFF, "DELETE FROM character_instance USING character_instance LEFT JOIN instance ON character_instance.instance = id WHERE map = ? and difficulty = ?", CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_DEL_EXPIRED_INSTANCE_BY_MAP_DIFF, "DELETE FROM instance WHERE map = ? and difficulty = ?", CONNECTION_ASYNC);
...
*/
    ...
    PrepareStatement(CHAR_SEL_PET_SLOTS_DETAIL, "SELECT owner, id, entry, level, name, loyalty FROM character_pet WHERE owner = ? AND slot >= ? AND slot <= ? ORDER BY slot", CONNECTION_ASYNC);
