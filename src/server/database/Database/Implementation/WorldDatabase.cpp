...
void WorldDatabaseConnection::DoPrepareStatements()
{
    ...
    */
    ...
    PrepareStatement(WORLD_SEL_CREATURE_POS_BY_ID, "SELECT guid, map, position_x, position_y, position_z, orientation FROM creature WHERE id = ?", CONNECTION_SYNCH);
    PrepareStatement(WORLD_SEL_CREATURE_COUNT_BY_ID, "SELECT COUNT(guid) FROM creature WHERE id = ?", CONNECTION_SYNCH);
