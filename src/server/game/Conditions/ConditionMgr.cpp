    ...
    switch (ConditionType)
    {
        ...
        case CONDITION_ITEM_EQUIPPED:
        {
            if (Player const* player = object->ToPlayer())
                ...
                condMeets = player->GetItemOrItemWithGemEquipped(ConditionValue1) != nullptr;
            break;
        }
                ...
                if (toUnit && unit)
                {
                    switch (static_cast<RelationType>(ConditionValue2))
                    {
                        ...
                        case RELATION_PASSENGER_OF:
                           // condMeets = unit->IsOnVehicle(toUnit); //TC LK
                            condMeets = true;
                            break;
...
//                    valid = addToLootTemplate(cond, LootTemplates_Milling.GetLootForConditionFill(cond->SourceGroup));
            ...
            if (!valid)
            {
                TC_LOG_ERROR("sql.sql", "%s Not handled or invalid grouped condition.", cond->ToString().c_str());
                delete cond;
            }
            else
            {
...
bool ConditionMgr::isConditionTypeValid(Condition* cond) const
{
    ...
    switch (cond->ConditionType)
    {
        ...
        case CONDITION_WOW_PATCH:
        {
            ...
            {
                TC_LOG_ERROR("sql.sql", "Patch condition %s has an invalid value in value1 %u (must be 0..10), skipping.", cond->ToString(true).c_str(), cond->ConditionValue1);
                return false;
            }
            if (cond->ConditionValue2 > WOW_PATCH_MIN)
            {
                TC_LOG_ERROR("sql.sql", "Patch condition %s has invalid argument %u (must be 0..10), skipped.", cond->ToString(true).c_str(), cond->ConditionValue2);
                return false;
            }
            break;
        }
