...
void Unit::RemoveAllControlled()
{
    ...
    if (GetCharmedGUID())
        TC_LOG_FATAL("entities.unit", "Unit %u is not able to release its charm %s", GetEntry(), ObjectGuid(GetCharmedGUID()).ToString().c_str());
        ...
        if (GetTypeId() == TYPEID_PLAYER)
        {
            if (Unit* charmer = GetCharmer())
            {
                ...
                if (Creature* creatureCharmer = charmer->ToCreature())
                {
                    ...
                    ;//sun: not an error... //TC_LOG_ERROR("misc", "Attempt to assign charm AI to player %s who is charmed by non-creature %s.", GetGUID().ToString().c_str(), GetCharmerGUID().ToString().c_str());
            }
        ...
        if (attacker && attacker->GetTypeId() == TYPEID_PLAYER) 
        {
            if (ObjectGuid minipet_guid = attacker->GetCritterGUID())
            {
                if (Creature* minipet = ObjectAccessor::GetCreature(*attacker, minipet_guid))
                    if (CreatureAI* ai = minipet->AI())
                        ai->MasterKilledUnit(pVictim);
            }
...
void Unit::SetFeared(bool apply)
{
    if (apply)
    {
       ...
       GetMotionMaster()->MoveFleeing(caster, fearAuras.empty() ? sWorld->getConfig(CONFIG_CREATURE_FAMILY_FLEE_DELAY) : 0, false);             // caster == NULL processed in MoveFleeing
    }
    else
    {
        if (!IsControlledByPlayer()) //sun: don't update for pets
...
void Unit::HandleSpellClick(Unit* clicker, int8 seatId)
{
...
float Unit::ComputeCollisionHeight() const
{
    ...
    if (IsMounted())
    {
        if (CreatureDisplayInfoEntry const* mountDisplayInfo = sCreatureDisplayInfoStore.LookupEntry(GetUInt32Value(UNIT_FIELD_MOUNTDISPLAYID)))
        {
            if (CreatureModelDataEntry const* mountModelData = sCreatureModelDataStore.LookupEntry(mountDisplayInfo->ModelId))
            {
                ...
                float const _collisionHeight = scaleMod * (mountModelData->MountHeight + modelData->CollisionHeight * modelData->Scale * displayInfo->scale * 0.5f);
                return _collisionHeight == 0.0f ? DEFAULT_COLLISION_HEIGHT : _collisionHeight;
            }
        }
    }
    ...
    float const _collisionHeight = scaleMod * modelData->CollisionHeight * modelData->Scale * displayInfo->scale;
    return _collisionHeight == 0.0f ? DEFAULT_COLLISION_HEIGHT : _collisionHeight;
}
...
Optional<ChaseRange> Unit::GetCombatRange() const
{
    return m_ChaseRange;
}
