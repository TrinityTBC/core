...
struct TC_GAME_API LootItem
{
    ...
    LootItem() : itemid(0), randomSuffix(0), randomPropertyId(0), count(0), conditionId(0), is_looted(false), is_blocked(false),
        freeforall(false), is_underthreshold(false), is_counted(false), needs_quest(false), follow_loot_rules(false)
    { };

    // Basic checks for player/item compatibility - if false no chance to see the item in the loot
    bool AllowedForPlayer(Player const * player) const;
    void AddAllowedLooter(Player const* player) { } //compat TC
    GuidSet GetAllowedLooters() const { return GuidSet(); } //compat TC
};
