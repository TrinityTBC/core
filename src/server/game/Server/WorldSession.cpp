...
void WorldSession::SendPacket(WorldPacket const* packet)
{
    ...
    else
    {
        ...
        TC_LOG_TRACE("network","Send all time packets count: " UI64FMTD " bytes: " UI64FMTD " avr.count/sec: %f avr.bytes/sec: %f time: %u",sendPacketCount,sendPacketBytes,float(sendPacketCount)/fullTime,float(sendPacketBytes)/fullTime,uint32(fullTime));
        TC_LOG_TRACE("network","Send last min packets count: " UI64FMTD " bytes: " UI64FMTD " avr.count/sec: %f avr.bytes/sec: %f",sendLastPacketCount,sendLastPacketBytes,float(sendLastPacketCount)/minTime,float(sendLastPacketBytes)/minTime);
...
void WorldSession::LogoutPlayer(bool Save)
{
    ...
    if (_player)
    {
        ...
        SendPacket( &data );
        TC_LOG_TRACE("network", "SESSION: Sent SMSG_LOGOUT_COMPLETE Message");
