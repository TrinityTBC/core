    ...
    if (mode & (AURA_EFFECT_HANDLE_REAL | AURA_EFFECT_HANDLE_REAPPLY))
    {
        ...
        if (apply)
        {
            ...
            case 34520: //Elemental Power Extractor
            {
                ...
                {
                    /* GameObject* elemPower = */ caster->ToPlayer()->SummonGameObject(183933, Position(m_target->GetPositionX(), m_target->GetPositionY(), (m_target->GetPositionZ() + 2.5f), m_target->GetOrientation()), QuaternionData(), (m_target->ToCreature())->GetRespawnTime() - m_target->GetMap()->GetGameTime());
...
void AuraEffect::HandleModUnattackable(AuraApplication const* aurApp, uint8 mode, bool apply) const
{
    ...
    m_target->ApplyModFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NON_ATTACKABLE_2, apply);
}
