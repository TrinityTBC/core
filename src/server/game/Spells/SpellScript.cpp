...
void SpellScript::EffectHandler::Call(SpellScript* spellScript, SpellEffIndex localEffIndex, int32& damage)
{
    (spellScript->*pEffectHandlerScript)(localEffIndex, damage);
}
...
bool SpellScript::_Validate(SpellInfo const* entry)
{
    ...
    for (auto & itr : OnDestinationTargetSelect)
        if (!itr.GetAffectedEffectsMask(entry)) 
        {
...
int32 SpellScript::GetHitDamage() const
{
    if (!IsInTargetHook())
    {
        TC_LOG_ERROR("scripts", "TSCR: Script: `%s` Spell: `%u`: function SpellScript::GetHitDamage was called, but function has no effect in current hook!", m_scriptName->c_str(), m_scriptSpellId);
        return 0;
    }
...
int32 SpellScript::GetHitHeal() const
{
    if (!IsInTargetHook())
    {
        TC_LOG_ERROR("scripts", "TSCR: Script: `%s` Spell: `%u`: function SpellScript::GetHitHeal was called, but function has no effect in current hook!", m_scriptName->c_str(), m_scriptSpellId);
        return 0;
    }
...
void SpellScript::SetHitHeal(int32 heal)
{
    if (!IsInTargetHook())
    {
        TC_LOG_ERROR("scripts", "TSCR: Script: `%s` Spell: `%u`: function SpellScript::SetHitHeal was called, but function has no effect in current hook!", m_scriptName->c_str(), m_scriptSpellId);
        return;
    }
...
void SpellScript::PreventHitEffect(SpellEffIndex effIndex)
{
    ...
    {
        TC_LOG_ERROR("scripts", "TSCR: Script: `%s` Spell: `%u`: function SpellScript::PreventHitEffect was called, but function has no effect in current hook!", m_scriptName->c_str(), m_scriptSpellId);
        return;
    }
...
void SpellScript::PreventHitDefaultEffect(SpellEffIndex effIndex)
{
    if (!IsInHitPhase() && !IsInEffectHook())
    {
        TC_LOG_ERROR("scripts", "TSCR: Script: `%s` Spell: `%u`: function SpellScript::PreventHitDefaultEffect was called, but function has no effect in current hook!", m_scriptName->c_str(), m_scriptSpellId);
        return;
    }
...
void SpellScript::SetCustomCastResultMessage(SpellCustomErrors result)
{
    ...
    {
        TC_LOG_ERROR("scripts", "TSCR: Script: `%s` Spell: `%u`: function SpellScript::SetCustomCastResultMessage was called while spell not in check cast phase!", m_scriptName->c_str(), m_scriptSpellId);
        return;
    }
...
AuraScript::EffectBase::EffectBase(uint8 _effIndex, uint16 _effName)
    : _SpellScript::EffectAuraNameCheck(_effName), _SpellScript::EffectHook(SpellEffIndex(_effIndex))
{ }

bool AuraScript::EffectBase::CheckEffect(SpellInfo const* spellEntry, SpellEffIndex effIndexToCheck)
{
...
void AuraScript::PreventDefaultAction()
{
    switch (m_currentScriptState)
    {
        ...
        case AURA_SCRIPT_HOOK_EFFECT_PROC:
            ...
            break;
        default:
            TC_LOG_ERROR("scripts", "TSCR: Script: `%s` Spell: `%u` AuraScript::PreventDefaultAction called in a hook in which the call won't have effect!", m_scriptName->c_str(), m_scriptSpellId);
            break;
    }
}
...
bool AuraScript::ModCharges(int8 num, AuraRemoveMode removeMode)
{
...
Unit* AuraScript::GetTarget() const
{
    switch (m_currentScriptState)
    {
        ...
        default:
            TC_LOG_ERROR("scripts", "TSCR: Script: `%s` Spell: `%u` AuraScript::GetTarget called in a hook in which the call won't have effect!", m_scriptName->c_str(), m_scriptSpellId);
    }
