...
TC_GAME_API float World::m_MaxVisibleDistanceForObject    = DEFAULT_VISIBILITY_DISTANCE;

TC_GAME_API float World::m_MaxVisibleDistanceInFlight     = DEFAULT_VISIBILITY_DISTANCE;
...
void World::LoadConfigSettings(bool reload)
{
    ...
    m_configs[CONFIG_CHAT_STRICT_LINK_CHECKING_SEVERITY] = sConfigMgr->GetIntDefault("ChatStrictLinkChecking.Severity", 1);
    m_configs[CONFIG_CHAT_STRICT_LINK_CHECKING_KICK] = sConfigMgr->GetIntDefault("ChatStrictLinkChecking.Kick", 0);
    ...
    m_configs[CONFIG_DETECT_POS_COLLISION] = sConfigMgr->GetBoolDefault("DetectPosCollision", true);
...
{
    SetMotd(sConfigMgr->GetStringDefault("Motd", "Welcome to TrinityTBC!") + std::string("\n") + std::string(GetPatchName()) + std::string(" is now live!"));
}
