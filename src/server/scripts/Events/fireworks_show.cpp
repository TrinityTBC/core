...
public:
    ...
    struct go_cheer_speakerAI : public GameObjectAI
    {
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            // Start
            if ((localTm.tm_min == 0 && localTm.tm_sec == 0) && !_started && (sGameEventMgr->IsActiveEvent(HOLIDAY_FIREWORKS_SPECTACULAR) || sGameEventMgr->IsActiveEvent(GAME_EVENT_NEW_YEAR)))
            {
            ...
            // Event is active
            if ((localTm.tm_min >= 0 && localTm.tm_sec >= 1 && localTm.tm_min <= 9 && localTm.tm_sec <= 59 && !_started) && (sGameEventMgr->IsActiveEvent(HOLIDAY_FIREWORKS_SPECTACULAR) || sGameEventMgr->IsActiveEvent(GAME_EVENT_NEW_YEAR)))
            {
                _events.ScheduleEvent(EVENT_FIRE, 1s);
                _started = true;
            }
            ...
            if ((localTm.tm_min == 10 && localTm.tm_sec == 30 && localTm.tm_hour == 0) && sGameEventMgr->IsActiveEvent(GAME_EVENT_NEW_YEAR) && _big == true)
            {
                _big = false;
                _events.ScheduleEvent(EVENT_CHEER, 1s);
                _events.ScheduleEvent(EVENT_FIRE, 1s);
