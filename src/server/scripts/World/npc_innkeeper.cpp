...
public:
    ...
    struct npc_innkeeperAI : public ScriptedAI
    {
        ...
        {
            if (sGameEventMgr->IsActiveEvent(HOLIDAY_HALLOWS_END) && !player->HasAura(SPELL_TRICK_OR_TREATED))
            {
        ...
        bool GossipSelect(Player* player, uint32 /*menuId*/, uint32 gossipListId) override
        {
            ...
            if (action == GOSSIP_ACTION_INFO_DEF + 1 && sGameEventMgr->IsActiveEvent(HOLIDAY_HALLOWS_END) && !player->HasAura(SPELL_TRICK_OR_TREATED))
            {
